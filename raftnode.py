...
class Node():
    ...
    def send(self, addr, route, message):
        url = addr + '/' + route
        try:
            reply = requests.post(
                url=url,
    ...
    def iv(self):
        ...
        if self.voteCount >= self.majority:
            ...
            self.startHeartBeat()
        # else: 
        #     print(f"{self.addr} becomes the follower of term {self.term}")
    ...
    def ask_for_vote(self, voter, term):
        ...
        while self.status == CANDIDATE and self.term == term:
            reply = self.send(voter, route, message)
            if reply:
    ...
    def update_follower_commitIdx(self, follower):
        ...
        reply = self.send(follower, route, first_message)
        if reply and reply.json()["commitIdx"] < self.commitIdx:
            
            reply = self.send(follower, route, second_message)
    ...
    def send_heartbeat(self, follower):
        ...
        while self.status == LEADER:
            start = time.time()
            reply = self.send(follower, route, message)
            if reply:
    ...
    def spread_update(self, message, confirmations=None, lock=None):
        for i, each in enumerate(self.fellow):
            r = self.send(each, "heartbeat", message)
